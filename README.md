...
**2. 저장 위치** — 비워두면 원본 파일과 같은 폴더에 저장한다.
...
- **중지 버튼은 파일 경계에서만 듣는다.** 한 파일 처리 중에는 끊기지 않는다.
- **Whisper(local) 받아쓰기는 파일 단위로만 진행률이 보인다.** 처리 중에는 진행바가 멎어 있다.
- **화자 분리와 자막(.srt) 은 GUI 에서 지원하지 않는다.** 단일 화자 녹음을 전제로
  뺐다. Groq CLI 의 `--srt` 만 남아 있다.
