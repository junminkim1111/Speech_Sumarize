...
DEFAULT_PROMPT = """다음은 녹음을 받아쓴 대본입니다. 읽기 좋은 정리본으로 만들어 주세요.

마크다운으로 작성하되, 형식은 내용에 맞게 정하세요. 회의면 논의와 결정 사항과
할 일이 드러나게, 강의나 발표면 개념과 흐름이 드러나게 하면 됩니다.
맨 위에 한두 문장짜리 요약을 두고, 그 아래에 본문을 정리하세요.
할 일이나 기한이 나오면 빠뜨리지 말고, 결론이 안 난 것도 그렇게 표시하세요.
"""
...
- 분량은 공백 포함 1500자에서 2500자 사이로 맞춥니다.
- 대본에 없는 내용을 지어내지 마세요. 불확실하면 그렇게 표시하세요.
- 받아쓰기 오류로 보이는 단어는 문맥에 맞게 보정해서 이해하되, 확신이 없으면 원문을 괄호로 병기하세요.
